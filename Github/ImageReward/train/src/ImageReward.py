...
class ImageReward(nn.Module):
    ...
    def encode_pair(self, batch_data):
        ...
        text_ids_better, text_ids_worse, text_mask_better, text_mask_worse, img = batch_data['text_ids_better'], batch_data['text_ids_worse'], batch_data['text_mask_better'], batch_data['text_mask_worse'], batch_data['img']
        text_ids_better = text_ids_better.view(text_ids_better.shape[0], -1).to(self.device) # [batch_size, seq_len]
        text_mask_better = text_mask_better.view(text_mask_better.shape[0], -1).to(self.device) # [batch_size, seq_len]
        text_ids_worse = text_ids_worse.view(text_ids_worse.shape[0], -1).to(self.device) # [batch_size, seq_len]
        text_mask_worse = text_mask_worse.view(text_mask_worse.shape[0], -1).to(self.device) # [batch_size, seq_len]
        img = img.to(self.device) # [batch_size, C, H, W]

        image_embeds = self.blip.visual_encoder(img)
        image_atts = torch.ones(image_embeds.size()[:-1], dtype=torch.long).to(self.device)
        
        # encode better emb
        emb_better = self.blip.text_encoder(text_ids_better,
                                            attention_mask = text_mask_better,
        ...
        # encode worse emb
        emb_worse = self.blip.text_encoder(text_ids_worse,
                                            attention_mask = text_mask_worse,
                                            encoder_hidden_states = image_embeds,
                                            encoder_attention_mask = image_atts,
                                            return_dict = True,
                                           ).last_hidden_state
