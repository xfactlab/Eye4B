...
from Github.Polos.polos.models.estimators.estimator_base import Estimator
from Github.Polos.polos.modules.feedforward import FeedForward
from Github.Polos.polos.modules.scalar_mix import ScalarMixWithDropout
from torchnlp.utils import collate_tensors
import Github.Polos.polos.clip as clip
import torch
...
class PolosEstimator(Estimator):
    ...
    def forward(
        ...
        **kwargs
        ...
        scores = []
        print(len(ref_sentemb_list))
        if reference_free:
            cos = F.cosine_similarity(imgs_clip, mts_clip, eps=0)
            cos[cos < 0.] = 0.
            clip_score = 2.5 * cos
            print(clip_scores.shape)
            print(hi)
            score = clip_score.unsqueeze(0)
