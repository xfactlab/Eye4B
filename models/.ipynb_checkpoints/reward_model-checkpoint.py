...
class RewardModelOutput(ModelOutput):
    rewards: Tensor = field(default=None, metadata={"help": "The rewards."})
...
class RewardModel(transformers.PreTrainedModel):
    ...
    def forward(self, input_ids, attention_mask=None, images=None, return_dict=True, **kwargs):
...
class RewardModelTrainer(transformers.Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        images = images.unsqueeze(1).repeat(1, input_ids.size(1), 1, 1, 1, 1)
        images = einops.rearrange(images, "b n t h w c -> (b n) t h w c")

        num_candidates, num_pairs = input_ids.size(1), choice.size(1)
        input_ids_flat, attention_mask_flat = tuple(einops.rearrange(x, "b c l -> (b c) l") for x in (input_ids, attention_mask))
        outputs = model(input_ids=input_ids_flat, attention_mask=attention_mask_flat, images=images)
